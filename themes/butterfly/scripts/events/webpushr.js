...
hexo.on("generateAfter", async function (post) {
    ...
    var JSONFeed = {
        ...
        cover1: newPost.cover,
        // tags: newPost.tags.data.map(function (v) {
        //     return v.name;
        // }),
        categories1: newPost.categories.data.map(function (v) {
            return v.name;
        }),
    };
    fs.writeFile(
        ...
        function (err, data) {
            logger.info("Generated: newPost.json");
        }
    );
});
...
hexo.on("deployAfter", async function (post) {
    ...
    // );
    const url = 'https://blog.ccknbc.cc/newPost.json';
    console.log(url); // "http://localhost/apiv1/events"
    console.log(typeof url); // string
    fetch(url).then(res => res.json());
    newPostOnlineSite = url;
    var newPostOnlineSite = JSON.parse(JSON.stringify(newPostOnlineSite));
    ...
    // });
    if(newPostLocal.categories1 == "工作")
    {
        var topic = "484223"
    }
    if(newPostLocal.categories1 == "博客")
    {
        var topic = "484224"
    }
    if(newPostLocal.categories1 == "工具")
    {
        var topic = "484225"
    }
    if(newPostLocal.categories1 == "生活")
    {
        var topic = "484226"
    }
    if(newPostLocal.categories1 == "音乐")
    {
        var topic = "484227"
    }
    if(newPostLocal.categories1 == "学习")
    {
        var topic = "484229"
    }
...
hexo.on("generateAfter", async function (post) {
    fs.writeFile(
        ...
        function (err, data) {
            logger.info("Generated: webpushr-sw.js");
        }
    );
});
